...
export const SingBoxService = GObject.registerClass({
    ...
    /**
     * 拿到单元的对象路径，顺带把属性订阅挂上。第一次调用时解析，之后复用。
     */
    async _getUnitPath() {
    ...
    destroy() {
        ...
        if (this._subscribed) {
            try {
                // 不用 this._call：紧接着的 cancel() 会把带 cancellable 的请求
                // 一起废掉，这条退订就发不出去了。
                this._bus.call(
                    BUS_NAME, MGR_PATH, MGR_IFACE, 'Unsubscribe', null, null,
                    Gio.DBusCallFlags.NONE, -1, null)?.catch?.(() => {});
            } catch (_error) {
                // 总线已经没了
            }
            this._subscribed = false;
